...
export function register() {
  ...
  if (!pubUrl || !pubAnon) {
    console.error(
      "[clubora] Missing NEXT_PUBLIC_SUPABASE_URL or NEXT_PUBLIC_SUPABASE_ANON_KEY in this Vercel environment — sessions and data access will fail.",
    );
  }

  if (!trim(process.env.SUPABASE_SERVICE_ROLE_KEY)) {
    console.error(
      "[clubora] Missing SUPABASE_SERVICE_ROLE_KEY in this Vercel environment — admin server paths (tasks, notifications, audit, some joins) will error when used.",
    );
  }
  ...
  if (!upstashOk) {
    console.warn(
      "[clubora] Upstash Redis env vars missing on Vercel — rate limits use in-memory fallback per instance (weak under serverless concurrency). Set UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN for preview and production.",
    );
  }
}
