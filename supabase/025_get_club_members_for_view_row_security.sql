-- ─── 025: Roster RPC must see profiles under RLS ─────────────────────────────
-- Supabase still applies RLS to joined tables inside some SECURITY DEFINER paths,
-- so get_club_members_for_view could return null full_name / email for everyone.
-- SET row_security = off for this function only; access is still scoped by
-- is_club_member(target_club_id, auth.uid()).
